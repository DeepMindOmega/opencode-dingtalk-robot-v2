...
def send_group_message(conv_id, content, token=None, msg_type="text", title=None):
    ...
    if msg_type == "text":
        ...
        msg_param = {"title": title or "消息", "text": content}
...
@retry_on_failure()
def send_private_message(user_id, content, token=None, msg_type="text", title=None):
    if not token:
        token = get_access_token()
...
def upload_media(file_path: str, token=None) -> Optional[str]:
    ...
    boundary = (
        "----WebKitFormBoundary" + hashlib.md5(str(time.time()).encode()).hexdigest()
    )
    ...
    body += f"--{boundary}\r\n".encode()
    body += f'Content-Disposition: form-data; name="media"; filename="{os.path.basename(file_path)}"\r\n'.encode()
    body += b"\r\n"
    body += file_content
    body += b"\r\n"
    body += f"--{boundary}--\r\n".encode()
...
class DingTalkHandler(dingtalk_stream.ChatbotHandler):
    async def process(self, callback: dingtalk_stream.CallbackMessage):
        ...
        task_id = qm.add_task(user_id, user_nick, text, conv_type, conv_id)
    ...
    while True:
        try:
            ...
            for task_id, result in results.items():
                ...
                if conv_type == "2":
                    if conv_id:
                        logger.info(f"[群聊] 发送到钉钉: conv_id={conv_id}")
