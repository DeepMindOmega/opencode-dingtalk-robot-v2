...
def run_opencode(
    message, continue_session=False, images=None, timeout=120, session_id=None
):
...
• **会话累计**: {total_tokens:,}"""
    ...
    # 如果响应提到截图/图片，说明已显示，不要尝试发送文件
    if has_screenshot or has_image_response:
        if generated_images:
            logger.info(
                f"[图片] 检测到 {len(generated_images)} 个新文件，但响应已说明截图已显示"
            )
        generated_images = []  # 清空，不尝试发送

    # 只有当 OpenCode 实际生成独立图片文件时才发送
    if generated_images:
        print(f"  → 生成独立图片: {len(generated_images)} 张")
